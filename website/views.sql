DROP VIEW IF EXISTS _cols;
CREATE VIEW _cols AS
  SELECT
      cv.id
    , db.name AS "db_name"
    ...
    , col.id AS column_id
    , column_type.name AS column_type
    , cv.nullable
    , note.note
    , v.is_current
    , license.license
    , license_url.id AS license_url_id
    , license_url.url AS license_url
    , license.attribution
  ...
  LEFT OUTER JOIN urls AS license_url ON cv.url_id = license_url.id
